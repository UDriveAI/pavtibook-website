...
class _SubscriptionUsageScreenState extends State<SubscriptionUsageScreen> {
  ...
  Future<void> _handleUpgrade(String planType) async {
    ...
    final int receiptLimit = planType == 'monthly'
        ? (config['monthly_receipts'] ?? 150)
        : (config['yearly_receipts'] ?? 2000);
    final int usersLimit = planType == 'monthly'
        ? (config['monthly_users'] ?? 3)
        : (config['yearly_users'] ?? 10);

    final monthlyPrice = config['monthly_price'] ?? 99;
    final savings = (monthlyPrice * 12) - price.toInt();

    final confirm = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Text(planType == 'monthly'
            ? 'Upgrade to Monthly'
            : 'Upgrade & Save ₹$savings'),
        content: Text(
            'Are you sure you want to switch to the ${planType == 'monthly' ? 'Monthly Plan' : 'Yearly Plan'} for ₹${price.toStringAsFixed(0)}?\n\n'
            'This will update your receipt limit to $receiptLimit and user count to $usersLimit.'),
        actions: [
          TextButton(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              Expanded(
                child: _buildMonthlyPlanCard(context, sub, config),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: _buildYearlyPlanCard(context, sub, config),
              ),
            ],
          ),
          const SizedBox(height: 24),
  ...
  Widget _buildMonthlyPlanCard(BuildContext context, SubscriptionModel sub,
      Map<String, dynamic> config) {
    final isActive = sub.plan == 'monthly';
    final price = config['monthly_price'] ?? 99;
    final users = config['monthly_users'] ?? 3;
    final receipts = config['monthly_receipts'] ?? 150;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildPlanFeature(Icons.people_outline, '$users Team Members'),
            const SizedBox(height: 6),
            _buildPlanFeature(
                Icons.receipt_long_outlined, '$receipts Receipts/Month'),
            const SizedBox(height: 6),
            _buildPlanFeature(Icons.dashboard_outlined, 'Dashboard'),
            const SizedBox(height: 6),
            _buildPlanFeature(Icons.people_alt_outlined, 'Donor Database'),
            const SizedBox(height: 6),
            _buildPlanFeature(Icons.analytics_outlined, 'Reports'),
            const SizedBox(height: 6),
            _buildPlanFeature(Icons.share_outlined, 'WhatsApp Sharing'),
            const SizedBox(height: 16),
            if (isActive)
              Container(
              ...
              ElevatedButton(
                ...
                child: const Text('Upgrade to Monthly',
                    style:
  ...
  Widget _buildYearlyPlanCard(BuildContext context, SubscriptionModel sub,
      Map<String, dynamic> config) {
    final isActive = sub.plan == 'yearly';
    final price = config['yearly_price'] ?? 999;
    final users = config['yearly_users'] ?? 10;
    final receipts = config['yearly_receipts'] ?? 2000;

    final monthlyPrice = config['monthly_price'] ?? 99;
    final totalMonthlyVal = monthlyPrice * 12;
    final savings = totalMonthlyVal - price;
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
            color: Colors.amber.withOpacity(isActive ? 0.35 : 0.15),
      ...
      child: Card(
        ...
        shape: RoundedRectangleBorder(
          borderRadius: BorderRadius.circular(16),
          side: BorderSide(
            color: isActive ? Colors.green : Colors.amber[600]!,
            width: 2.2,
          ),
        ),
        child: Padding(
            ...
            children: [
              ...
              Container(
                ...
                child: Column(
                  children: [
                    Text(
                      'Monthly: ₹$monthlyPrice × 12 = ₹$totalMonthlyVal',
                      style: const TextStyle(
                          fontSize: 8,
                          color: Colors.grey,
              ...
              const SizedBox(height: 6),
              _buildPlanFeature(
                  Icons.receipt_long_outlined, '$receipts Receipts/Year'),
              const SizedBox(height: 6),
              _buildPlanFeature(Icons.star_outline, 'Everything in Monthly'),
              const SizedBox(height: 24),
              if (isActive)
                Container(
                  padding: const EdgeInsets.symmetric(vertical: 8),
                ...
                ElevatedButton(
                  ...
                  child: Text('Upgrade & Save ₹$savings',
                      style: const TextStyle(
  ...
  Widget _buildPlanFeature(IconData icon, String text) {
    return Row(
      children: [
        Icon(icon, size: 12, color: const Color(0xFFF47C20)),
        const SizedBox(width: 4),
        Expanded(
          child: Text(
            text,
            style: const TextStyle(fontSize: 9, color: Color(0xFF2E1C0C)),
            maxLines: 1,
            overflow: TextOverflow.ellipsis,
          ),
        ),
              ...
              children: [
                TableCell(
                    child: Padding(
                        padding: EdgeInsets.symmetric(vertical: 8),
                        child:
                            Text('Receipts', style: TextStyle(fontSize: 10)))),
                TableCell(
                    child: Center(
                        child: Padding(
                            padding: EdgeInsets.symmetric(vertical: 8),
